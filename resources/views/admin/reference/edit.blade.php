...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading"><h4>Editace reference</h4></div>
        <div class="panel-body">
            {!! Form::model($reference, array('method'=>'put' ,"files"=>true,'route' => array('admin.reference.update', $reference->id))) !!}
            ...
            <div class="row">
                @foreach($reference->files as $index=>$file)
                    <div class="col-md-4 col-xs-12">
                        <label for="{{$index}}-photo" class="card__img" style="background: url('{{ getPathToReference($reference->id,$file->filename)}} ')"></label>
                    </div>
                @endforeach
            </div>
