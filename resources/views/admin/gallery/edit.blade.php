...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading"><h4>Editace fotogalerie</h4></div>
        <div class="panel-body">
            {!! Form::model($gallery, array('method'=>'put' ,"files"=>true,'route' => array('admin.galerie.update', $gallery->id))) !!}
            ...
            <div class="row">
                <div class="col-xs-12">
                    <hr>
                    <h4>Smazat vybrané fotky</h4>
                </div>
                @foreach($gallery->files as $index=>$file)
                    <div class="col-md-4 col-xs-12">
                        <div class="checkbox text-center">
                            <input name="delete[]" value="{{$file->id}}" type="checkbox" id="{{$index}}-photo">
                        </div>
                        <label for="{{$index}}-photo" class="card__img" style="background: url('{{ getPathToGallery($gallery->id,$file->filename)}} ')"></label>
                    </div>
                @endforeach
            </div>
